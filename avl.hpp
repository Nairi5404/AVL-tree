...
template <typename T>
void AVL<T>::help_delete(Node*& ptr)
{
	if (ptr != nullptr)
	{
		help_delete(ptr->left);
		help_delete(ptr->right);
		delete ptr;
		ptr = nullptr;
	}
}
...
bool AVL<T>::compare(Node* root1,Node* root2) const
{
	...
	if (root1 != nullptr && root2 != nullptr && root1->value == root2->value)
	{
		bool t1 = false;
		bool t2 = false;
		t1 =  compare(root1->left, root2->left);
		t2 =  compare(root1->right, root2->right);
		return t1 && t2;
	}
	return false;
}
...
template <typename T>
int AVL<T>::help_max(int a, int b) const
{
	return (a > b) ? a : b;
}
